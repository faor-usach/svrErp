...
$link=Conectarse();
if($dato->accion == "ActualizaItems"){
    $SQL = "SELECT * FROM dcotizacion Where CAM = '$dato->CAM' and nLin = '$dato->nLin'";
    $bd=$link->query($SQL);
    if($row=mysqli_fetch_array($bd)){ 
        $actSQL="UPDATE dcotizacion SET ";
        if($dato->Moneda == 'U'){
            $actSQL.="unitarioUF            = '".$dato->unitarioUF.             "', ";
            $actSQL.="NetoUF                = '".$dato->NetoUF.                 "', ";
            $actSQL.="IvaUF                 = '".$dato->IvaUF.                  "', ";
            $actSQL.="TotalUF               = '".$dato->TotalUF.                "', ";
        }
        if($dato->Moneda == 'P'){
            $actSQL.="unitarioP             = '".$dato->unitarioP.              "', ";
            $actSQL.="Neto                  = '".$dato->Neto.                   "', ";
            $actSQL.="Iva                   = '".$dato->Iva.                    "', ";
            $actSQL.="Bruto                 = '".$dato->Bruto.                  "', ";
        }
        if($dato->Moneda == 'D'){
            $actSQL.="unitarioUS            = '".$dato->unitarioUS.             "', ";
            $actSQL.="NetoUS                = '".$dato->NetoUS.                 "', ";
            $actSQL.="IvaUS                 = '".$dato->IvaUS.                  "', ";
            $actSQL.="TotalUS               = '".$dato->TotalUS.                "', ";
        }
