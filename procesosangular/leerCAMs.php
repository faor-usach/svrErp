...
$link=Conectarse();
	...
	$fechaHoy = date('Y-m-d');
	$fecha60dias 	= strtotime ( '-90 day' , strtotime ( $fechaHoy ) );
...
$SQL = "Select * From cotizaciones Where fechaInicio = '0000-00-00' and Estado = 'E' or Estado = '' Order By tpEnsayo Desc, Estado Desc";
...
if(isset($dato->usrCotizador)){
	if($dato->usrCotizador == 'Todos'){
		...
		if($dato->usrCotizador == 'Baja'){
			$agnoBaja = $agnoAct - 1;
			$SQL = "Select * From cotizaciones Where Estado = 'C' and year(fechaCotizacion) >= '$agnoBaja' Order By tpEnsayo Desc, Estado Desc";
		}else{
			$SQL = "Select * From cotizaciones Where usrCotizador = '$dato->usrCotizador' and fechaInicio = '0000-00-00' and Estado = 'E' or Estado = '' Order By tpEnsayo Desc, Estado Desc";
		}
	}
}
...
while($rs = mysqli_fetch_array($bd)){
	...
	if($rsc = mysqli_fetch_array($bdc)){
		...
		for($i=1; $i<=$dh; $i++){
			...
			$SQLdf = "SELECT * FROM diasferiados where fecha = '$ft'";
			$bddf = $link->query($SQLdf);
			if($rsdf = mysqli_fetch_array($bddf)){
				$dh++;
				$dd++;
		...
		$outp.= '"contactoRecordatorio":' 	.json_encode($rs["contactoRecordatorio"], JSON_HEX_TAG | JSON_HEX_APOS | JSON_HEX_QUOT | JSON_HEX_AMP | JSON_UNESCAPED_UNICODE) 	.',';
