...
$res = '';
$link=Conectarse();
$SQL = "SELECT * FROM cotizaciones Where CAM = '$dato->CAM' and RAM = '$dato->RAM'";  
$bd=$link->query($SQL);
if($row=mysqli_fetch_array($bd)){ 
   ...
   if($rowCl=mysqli_fetch_array($bdCl)){
    ...
    $Descripcion = utf8_decode($row['Descripcion']);
        ...
        $res.= '{"CAM":"'.			    $dato->CAM.				    '",';
        $res.= '"mail_destinatario":"'. $rowCc['Email'].		    '",';
        $res.= '"Cliente":"'.           $Cliente.		            '",';
        $res.= '"fInicio":"'.           $fInicio.		            '",';
        $res.= '"horaPAM":"'.           $horaPAM.		            '",';
        $res.= '"fTermino":"'.          $ft.		                '",';
        $res.= '"emailCotizador":"'.    $emailCotizador.		    '",';
    ...
    //$loc = "Location: plataformaCotizaciones.php?eviarCorreo='SI'";
    //header($loc);
