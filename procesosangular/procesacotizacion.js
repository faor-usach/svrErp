    ...
    app.run(function($rootScope){
        ...
        $rootScope.btnCotZero = 'Cotización Zero';
        $rootScope.btnCotZeroColor = 'btn btn-warning'; 
        $rootScope.fechaCotizacion  = new Date();

        $rootScope.Descripcion = '';
        $rootScope.obsServicios = '';
        $rootScope.Observacion = '';
    ...
    app.controller('CtrlCotizaciones', function($scope, $http, $interval) {
        $interval(function () {
        ...
        $scope.cotizacionZero = function() {
            if($scope.CAM > 0){
                if($scope.btnCotZero == "Cotización Zero"){
                    $http.post('grabarCAMZero.php', {
                                                    CAM:$scope.CAM, 
                                                    Rev:$scope.Rev
                    })
                    .then(function (response) {
                        $scope.resGuardaZero = 'OK';
                    ...
                    $scope.btnCotZero = 'Recalcular';
                    $rootScope.btnCotZeroColor = 'btn btn-danger';
                }else{
                    $scope.btnCotZero = 'Cotización Zero';
                    $rootScope.btnCotZeroColor = 'btn btn-warning';
                }
            }
        }
        ...
        $scope.leerValoresReferenciales = function() {
            $http.post("leerbuscaUltimaUF.php")  
            .then(function(response){  
                $scope.valorUFRef      = response.data.valorUF;
            }, function(error) {
                $scope.errors = error.message;
            });
        };
        ...
        $scope.leerValoresReferenciales();
        ...
        $scope.loadCAM = function(CAM){
            ...
            if(CAM == 0){
                ...
                $scope.Validez = 30;
                $scope.dHabiles = 30;
                $scope.valorUF  = $scope.valorUFRef;
            }else{
                
                $http.post("leerSeguimientoCotizacion.php",{CAM:CAM})  
                .then(function(response){
                    //alert('CAM '+response.data.RAM);
                    $scope.conCAM = true;
        ...
        $scope.gGG = function(form){

            $http.post("buscarUltimaCAMPrueba.php", {   
                RutCli:$scope.RutCli,
                CAM:$scope.CAM
                })  
            .then(function(response){ 
        ...
        $scope.grabarCotizacion = function(){
            ...
            if($scope.Descipcion == ""){
                alert('Indefinida '+$scope.Descripcion);
            }
