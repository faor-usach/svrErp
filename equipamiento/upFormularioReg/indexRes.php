...
/*
		$nombre_FMAN 	= $_FILES['FMAN']['name'];
		$tipo_FMAN 		= $_FILES['FMAN']['type'];
		$tamano_FMAN 	= $_FILES['FMAN']['size'];
		$desde_FMAN		= $_FILES['FMAN']['tmp_name'];
		
		if($nombre_FMAN){
			$directorioPOC='../../archivo/POC-27/Formularios/'.'Reg-2707';
			if(!file_exists($directorioPOC)){
				mkdir($directorioPOC,0755);
			}
				
			if($tipo_FMAN == "application/pdf") {
				$newFMAN = 'Reg-Man-'.$codigo.'-'.date('Y-m-d').'.pdf';
				if(move_uploaded_file($desde_FMAN, $directorioPOC."/".$newFMAN)){ 
...
*/
		...
		if($row=mysqli_fetch_array($bdEnc)){
			do{
				...
				if($nombre_pdf){
					echo 'PDF '.$nombre_pdf;
					$sqlPOC = "SELECT * FROM docFormPOC Where Formulario = '".$row['Formulario']."'";
					$bdPOC=$link->query($sqlPOC);
					if($rowPOC=mysqli_fetch_array($bdPOC)){
						//$directorioPOC='../../archivo/'.$rowPOC['Referencia'].'/Formularios/'.'Reg-'.$row['Formulario'];
						$directorioPOC='http://servidorerp/erp/archivo/'.$rowPOC['Referencia'].'/Formularios/'.'Reg-'.$row['Formulario'];
						if(!file_exists($directorioPOC)){
							mkdir($directorioPOC,0755);
						}
						if($tipo_pdf == "application/pdf") {
							$newpdf = 'Reg-'.$codigo.'-'.date('Y-m-d').'.pdf';
