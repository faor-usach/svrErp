				...
				if($rowEns=mysql_fetch_array($bdEns)){
					do{
						...
						if($rowTe=mysql_fetch_array($bdTe)){
							...
							// Registrar en OTAMs
							if($ensPrevios != $cEnsayos){
								$ensIni   	= $tEnsayos + 1;

								$regNuevo = 'No';
								$ensFin	  = $tEnsayos + ($cEnsayos - $ensPrevios);
								
								if($ensPrevios >= 1){ // OJO DEJAR EN UNO REVISAR EL WHILE
									// Si tEnsayos > tEnsayosOt HAY QUE INSERTAR UN REGISTRO
									if($cEnsayos > $ensPrevios){
										// Insertar Registro
										$nInsertar = $cEnsayos - $ensPrevios; // 3 - 2 = 1
										$ultOtam   = $tEnsayos; // = 2
										...
										if($rowOT=mysql_fetch_array($bdOT)){
											$uOtam = $rowOT['Otam']; // 1000-Q02
										}
										$sigOtam = $tEnsayos + $nInsertar; // 2 + 1 = 3
										$actOtam = $tEnsayos; // = 2
										$Otams  = $RAM.'-'.$rowEns['Suf'].$actOtam; // 1000-T02
										if($actOtam<10){
											$Otams = $RAM.'-'.$rowEns['Suf'].'0'.$actOtam;
										}
										
										$SwOtam = false;
										while($SwOtam == false){
											$sOtams = $RAM.'-'.$rowEns['Suf'].$sigOtam; // 1000-T03
											if($sigOtam<10){
												$sOtams = $RAM.'-'.$rowEns['Suf'].'0'.$sigOtam;
											}

											$actSQL="UPDATE OTAMs SET ";
											$actSQL.="Otam		 = '".$sOtams."' "; // T03
											$actSQL.="WHERE Otam = '".$Otams."'"; // T02
											$bdOt=mysql_query($actSQL);

											$sigOtam--; 
											$actOtam--;
											$Otams  = $RAM.'-'.$rowEns['Suf'].$actOtam; // 1000-T06
											if($actOtam<10){
												$Otams = $RAM.'-'.$rowEns['Suf'].'0'.$actOtam;
											}
											if($Otams == $uOtam){
										...
										// Borrar Ensayos
										$ensBorrar 	 = $ensPrevios - $cEnsayos;
										$hastaEnsayo = $ensPrevios;
										$bdOT=mysql_query("SELECT * FROM OTAMs Where idItem = '".$idItem."' and Otam Like '%".$rowEns['Suf']."%' Order By Otam Desc");
										if($rowOT=mysql_fetch_array($bdOT)){
											$uOtam   = $rowOT['Otam'];
											$desdeEnsayo = ($ensPrevios - $ensBorrar) + 1;
										}
										for($i = $desdeEnsayo; $i <= $hastaEnsayo; $i++){
											$otamBorrar = $RAM.'-'.$rowEns['Suf'].$i;
											if($i<10){
												$otamBorrar = $RAM.'-'.$rowEns['Suf'].'0'.$i;
											}
											//$bdOT = mysql_query("Delete From OTAMs Where Otam = '".$otamBorrar."'");
										}
										
											?>
												<script>
													alert('<?php echo "Borrar ".$otamBorrar." de ".$tEnsayos." Ultimo Ensayo Item = ".$uOtam." Desde = ".$desdeEnsayo." Hasta = ".$hastaEnsayo; ?>');
												</script>
											<?php
										
									}
								} // Fin Previos
								// Si tEnsayos < tEnsayosOt BORRAR REGISTRO
