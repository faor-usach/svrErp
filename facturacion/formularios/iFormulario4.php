<?php
	...
	if($rowSol=mysql_fetch_array($bdSol)){
		...
		$pdf->SetFont('Arial','B',10);
		$pdf->SetXY(10,37);
		$pdf->Cell(45,5,utf8_decode('FORMULARIO Nº 4'),1,0,'L');
		...
		if($rowDep=mysql_fetch_array($bdDep)){
			...
			$pdf->Cell(110,5,strtoupper($rowDep['NomDirector']).' ('.utf8_decode($rowDep['Cargo']).')',0,0);
		}		
	...
	$vDir = 'Y://AAA/Financiero/Solicitud-Facturas/'.$nSolicitud;
	$vDir = 'Y://AAA/LE/FINANZAS/'.$agnoActual.'/SOLICITUD-FACTURA/'.$nSolicitud;
	if(!file_exists($vDir)){
		mkdir($vDir);
	}
