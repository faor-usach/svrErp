...
app.controller('ctrlGastos', function($scope, $http) {
    ...
    $scope.loadGastos = function(Formulario, cIva, IdProyecto){
        ...
        $http.post("registraDataGastos.php",{
            ...
            accion      : "lecturaTablaGastos"
        })   
        .then(function(response){
            $scope.dataGastos = response.data.records;
        }, function(error) {
            alert(error.message);
            $scope.errors = error.message;
        });
    ...
    $scope.imprimirSolicitud = function(){
        ...
        if($scope.nInforme > 0){
            ...
            alert('Se generó solicitud reembolso...'+$scope.nInforme);
    ...
    $scope.buscaNumSolEmiteFormulariosinAnexo = function(){
        alert($scope.Formulario+' '+$scope.cIva+' '+$scope.IdProyecto+' '+$scope.Concepto);
