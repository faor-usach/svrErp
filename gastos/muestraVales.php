			<div id="accordion">
				<?php
					...
					$link=Conectarse();
					$bdGto=$link->query("SELECT * FROM vales Where $filtro Order By fechaVale Desc");
					...
					if ($row=mysqli_fetch_array($bdGto)){
						do{
							...
							if($fd[1] != $mes){
								...
								$mes = $fd[1];
								?>
								<h3 style="font-size:14px;"><?php echo $Mes[intval($fd[1])]. '<span style="float:right;"> $ '.number_format($tMesA[intval($mes)], 0, ',', '.').'</span>'; ?>
