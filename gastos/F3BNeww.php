...
if($Formulario == "F3B"){
	...
	$pdf->Cell(100,1,'SOCIEDAD DE DESARROLLO TECNOLÓGICO DE LA USACH LTDA.',0,2,'C');
	$pdf->Ln(10);
	...
	$pdf->Cell(10,5,':',0,0);
	$pdf->Cell(80,5,'SEÑOR '.strtoupper($rowDep['NomDirector']).' (Jefe del Proyecto)',0,0);
	...
	if($row=mysqli_fetch_array($bdPr)){
		...
		$pdf->Ln(5);
		$pdf->Cell(70,5,'CÓDIGO DEL PROYECTO',1,0);
		...
		$pdf->Ln(5);
		$pdf->Cell(70,5,'CORREO ELECTRÓNICO',1,0);
		$pdf->Cell(110,5,$Email,1,0);
		$pdf->Ln(5);
		$pdf->Cell(70,5,'MODALIDAD DE PAGO',1,0);
		$pdf->Cell(110,5,'TRANSFERENCIA ELECTRÓNICA',1,0);
		$pdf->Ln(5);
	...
	$pdf->SetXY(30,$nLn);
	$txt  = "La dirección comercial de SDT USACH LTDA: es Av. Libertador Bernardo O'Higgins N° 1611, sin embargo, ";
...
if($Formulario == "F7"){
	...
	$link=Conectarse();
			...
			$pdf->Cell(70,5,'A:',0,0);
			//$pdf->Cell(2,5,':',0,0);
			$pdf->Cell(110,5,'DIRECTOR EJECUTIVO SOCIEDAD DE DESARROLLO TECNOLÓGICO (SDT USACH)',0,0);
