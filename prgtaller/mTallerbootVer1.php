<?php
	...
	$link=Conectarse();
	$sqlm = "SELECT * FROM ammuestras Where Taller = 'on' and fechaTaller > '0000-00-00' and fechaTerminoTaller = '0000-00-00' Order By fechaTaller Desc";
	$bdm = $link->query($sqlm);
	if($rowm=mysqli_fetch_array($bdm)){
		$_SESSION['fechaUltimaMuestra'] = $rowm['fechaTaller'];
	}

	$sqlm = "SELECT * FROM ammuestras Where Taller = 'on' and fechaTaller > '0000-00-00' and fechaTerminoTaller = '0000-00-00' Order By fechaTaller Asc";
	$bdm = $link->query($sqlm);
	if($rowm=mysqli_fetch_array($bdm)){
...
<div class="row bg-primary text-white">
	...
	<div class="col-xl-8">
		<div class="row text-center">
			...
			<div class="col-xl-8"><h4>Planificación Taller Mecánico</h4></div>
...
<div class="row">
	<?php
		for($i=0; $i<=4; $i++){
			...
			?>
			<div class="col-xl-2 text-center">
				<table class="table table-hover bg-success text-white">
					<tbody>
				<?php
					$link=Conectarse();
					$item = '';
					$sqlm = "SELECT * FROM ammuestras Where Taller = 'on' and (fechaTaller = '$fSem' or fechaHasta = '$fSem') and fechaTerminoTaller = '0000-00-00' Order By idItem";
					//echo $sqlm;
					$bdm = $link->query($sqlm);
