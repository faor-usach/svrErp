<?php
	...
	if($rowRAM=mysqli_fetch_array($bdRAM)){
		...
		if($rowRAM['nSolTaller']>0){
			...
			$sqlUs = "SELECT * FROM Usuarios Where usr = '".$rowRAM['ingResponsable']."'";
			$bdUs=$link->query($sqlUs);
			if($rowUs=mysqli_fetch_array($bdUs)){
				$usuariosRes = $rowUs['usuario'];
				$usuariosRec = $rowUs['usuario'];
			}
			...
			$pdf->SetFont('Arial','B',12);
			$pdf->SetXY(40,$ln);
			$pdf->Cell(144,7,utf8_decode($usuariosRes),0,0,'L');
			...
			while($rowMu=mysqli_fetch_array($bdMu)){
					...
					if($nOtam > 4 and $nPagServ == 1){
						...
						$pdf->SetXY(92,$ln);
						$pdf->Cell(10,7,': '.$rsot['tecRes'],0,0,'L');
						$ln += 7;
						$pdf->Line(10, $ln, 184, $ln);
					...
					if($rowMu['idItem'] != $ultItem){
						...
						$pdf->SetFont('Arial','B',12);
						$pdf->SetXY(40,$ln);
						$pdf->Cell(144,7,$rowMu['idItem'].' '.$rowMu['idMuestra'],0,0,'L');
						$pdf->SetTextColor(0,0,0);
						$ln += 7;
						$pdf->Line(10, $ln, 184, $ln);
						...
						$pdf->SetXY(10,$ln);
						$pdf->SetFont('Arial','',8);
						$pdf->MultiCell(180,4,utf8_decode($rowMu['Objetivo']),0,'L');
			...
			if($nOtam <= 4){
				...
				$pdf->SetFont('Arial','B',12);
				$pdf->SetXY(10,$ln);
				$pdf->Cell(17,7,'Observaciones : '. utf8_decode($rowRAM['Obs']),0,0,'L');
				...
				$pdf->SetXY(92,$ln);
				$fd = explode('-', $fechaHoy);
				$pdf->Cell(10,7,': '.$fd[2].'/'.$fd[1].'/'.$fd[0],0,0,'L');
				$ln += 7;
				$pdf->Line(10, $ln, 184, $ln);
				...
				$pdf->Cell(100,7,'Persona que recibe el trabajo',0,0,'L');
				$pdf->SetXY(92,$ln);
				$pdf->Cell(10,7,': '.utf8_decode($usuariosRec),0,0,'L');
				$ln += 7;
				$pdf->Line(10, $ln, 184, $ln);
			}
	...
	$NombreFormulario = "Hoja-Numero-".$rowRAM['nSolTaller'].'-RAM-'.$RAM."-Salida.pdf";
