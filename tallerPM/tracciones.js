...
app.run(function($rootScope){
    ...
    $rootScope.ficherosTracciones       = false;
    $rootScope.resultadosTracciones     = false;
    $rootScope.muestraOtamsTracciones   = false;
});
...
app.controller('ctrlEspectometro', function($scope, $http) {
    ...
    $scope.TraccionesEnsayar = function(){
        $http.post('dataTracciones.php',{
            accion:     "dataTraccionesEnsayar"
        })
    ...
    $scope.mostrarTracciones = function(RAM){
        $scope.RAM = RAM;
        ...
        $http.post('dataTracciones.php',{
            RAM     : RAM,
            accion  : "mostrarTracciones"
        })
        .then(function (response) {
            $scope.dataTraccionesOtam = response.data.records; 
        }, function(error) {
            $scope.errors = error.message;
            alert($scope.errors);
        });
