				...
				if($rowInd=mysqli_fetch_array($bdInd)){
					...
					<div class="<?php echo $cIndice; ?>" style="width:60px; text-align:center; float:right;">
						...
						<?php 
							...
							$bdid=$link->query("SELECT * FROM tablaindicadores Where agnoInd = '".$fh[0]."' and mesInd = '".$fh[1]."'");
							if($rs=mysqli_fetch_array($bdid)){
								$TindDesc = $rs['indDesc'] + $rs['indDesc2'] + $rs['indDesc3'];
							}
							
							echo number_format($rowInd['indVtas'] - $TindDesc, 2, ',', '.');
						?>
					</div>
