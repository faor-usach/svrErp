		...
		<table border="1" cellpadding="0" cellspacing="0">
			...
			<tr style="font-weight:700; font-size:12px;" height="25">
  			...
  			<?php
				...
				//$bdFac=mysql_query("SELECT * FROM SolFactura Where nFactura > 0 && pagoFactura = '' Order By fechaFactura Asc");
				$bdFac=mysql_query("SELECT * FROM SolFactura Where nFactura > 0 and pagoFactura <> 'on' Order By fechaFactura Asc");
			...
			<tr style="font-weight:700; font-size:12px;" height="25">
  			...
  			<?php
				...
				$n = 0;
				//$bdFac=mysql_query("SELECT * FROM SolFactura Where year(fechaPago) = $Agno && month(fechaPago) = $Mes Order By fechaPago Asc");
				//$bdFac=mysql_query("SELECT * FROM SolFactura Where nFactura = 0 and Fotocopia = 'on' && Estado = 'I' Order By fechaFactura Asc");
				$bdFac=mysql_query("SELECT * FROM SolFactura Where nFactura = 0 and Fotocopia = 'on' && Estado = 'I' Order By fechaFactura Asc");
				if ($rowFac=mysql_fetch_array($bdFac)){
					do{ 
						$n++;
			...
			<tr style="font-weight:700; font-size:16px;" height="25">
  			...
  			<?php
				...
				$n = 0;
				//$bdFac=mysql_query("SELECT * FROM SolFactura Where year(fechaPago) = $Agno && month(fechaPago) = $Mes Order By fechaPago Asc");
				$bdFac=mysql_query("SELECT * FROM SolFactura Where nFactura = 0 and Fotocopia = ' ' and Estado = 'I' Order By fechaFactura Asc");
				if ($rowFac=mysql_fetch_array($bdFac)){
					do{ 
						$n++;
						$bdCli=mysql_query("SELECT * FROM clientes Where RutCli = '".$rowFac['RutCli']."'");
