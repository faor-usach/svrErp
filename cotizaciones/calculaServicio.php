<?php 	
	...
	$link=Conectarse();
	$bdSer=$link->query("SELECT * FROM Servicios Where nServicio = '".$nServicio."'");
	...
	if($nLin > 0){
		$Cantidad = 0.00;
		$bddCot=$link->query("SELECT * FROM dCotizacion Where CAM = '".$CAM."' and Rev = '".$Rev."' and Cta = '".$Cta."' && nLin = '".$nLin."'");
				...
				<td width="20%">
					<table width="100%" cellpadding="0" cellspacing="0" style="border:1px solid #ccc;">
						...
						<tr>
						  	...
						  	<td>
								<input style="font-size:18px; font-weight:700;" name="Total" id="Total" type="text" size="10" maxlength="10" value="<?php echo $Total; ?>" readonly>
							</td>
						</tr>
						<tr>
						  	<td>&nbsp;</td>
