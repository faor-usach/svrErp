...
<?php
	...
	date_default_timezone_set("America/Santiago");
				...
				$AgnoAnt = date('Y') - 1;
				$ultRAM = 0;
				$bdMu=$link->query("SELECT * FROM registroMuestras Where CAM = 0 and year(fechaRegistro) >= $AgnoAnt and situacionMuestra = 'R' Order By RAM Asc");
				if($rowMu=mysqli_fetch_array($bdMu)){
					$ultRAM = $rowMu['RAM'];
				}

				if($RutCli){
					//$bdEnc=$link->query("SELECT * FROM registroMuestras Where RutCli Like '%".$RutCli."%' and situacionMuestra != 'B' and situacionMuestra != 'P' Order By RAM Desc");
					$bdEnc=$link->query("SELECT * FROM registroMuestras Where RutCli Like '%".$RutCli."%' and situacionMuestra != 'B' Order By RAM Desc");
				}else{
					$ultRAM = 10490;
					//$bdEnc=$link->query("SELECT * FROM registroMuestras Where situacionMuestra != 'B' and RAM >= $ultRAM Order By RAM Desc");
					//$bdEnc=$link->query("SELECT * FROM registroMuestras Where (situacionMuestra = 'R' or situacionMuestra = 'P') and RAM > $ultRAM Order By RAM Desc");
					$bdEnc=$link->query("SELECT * FROM registroMuestras Where (situacionMuestra = 'R' or situacionMuestra = 'P')  Order By RAM Desc");
				}
...
*/						
						$muestraRAM = 'Si';
						$bdCot=$link->query("SELECT * FROM cotizaciones Where RAM = '".$row['RAM']."' and Fan = '".$row['Fan']."'");
						if($rowCot=mysqli_fetch_array($bdCot)){
							...
							if($rowCot['Estado'] == 'T' or $rowCot['Estado'] == 'C' or $rowCot['Estado'] == 'P' or $rowCot['Estado'] == 'N'){
								...
								//if($row['CAM'] > 0){
									...
									$actSQL.="WHERE RAM	= '".$cerrarRAM. "' and Fan = '".$cerrarFan."'";
...
/*						}
						if($row['CAM']){
							$bdCot=$link->query("SELECT * FROM cotizaciones Where RAM = '".$row['RAM']."'");
							if($rowCot=mysqli_fetch_array($bdCot)){
...
*/							
						if($muestraRAM == 'Si'){
							...
							echo '	<td width="40%">';
										if($row['CAM']>0){
											$bdCot=$link->query("SELECT * FROM Cotizaciones Where CAM = '".$row['CAM']."'");
											if($rowCot=mysqli_fetch_array($bdCot)){
							...
							echo ' 	</td>';
							$bdMue=$link->query("SELECT * FROM Cotizaciones Where RutCli = '".$row['RutCli']."' and Estado = 'E'");
