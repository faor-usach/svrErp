<?php
	...
	if(isset($_GET['dBuscar'])) { $dBuscar  = $_GET['dBuscar']; 	}
				...
				if($dBuscar){
					$bdEnc=mysql_query("SELECT * FROM Servicios Where Servicio Like '%".$dBuscar."%' Order By nServicio Asc");
				}else{
					$bdEnc=mysql_query("SELECT * FROM Servicios Order By nServicio Asc");
				}
