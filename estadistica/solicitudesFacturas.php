<?php
	...
	if(isset($_POST['borrarSolicitud'])){ 
		...
		if($rowSol=mysql_fetch_array($bdSol)){
			...
			$bdDet=mysql_query("DELETE FROM detSolFact WHERE nSolicitud = '".$nSolicitud."'");
		}
		mysql_close($link);
		$RutCli 	= "";
		$nSolicitud = "";
...
?>
			...
			<div id="BarraFiltro">
				<?php 
					...
					$bdHon=mysql_query("SELECT * FROM SolFactura Where $filtroFacturas");
					...
					<span style="font-size: 12px; margin: 3px; color:#fff; font-family:Verdana, Arial, Helvetica, sans-serif;">
						Facturación en Docencia <?php echo '<span style="font-size:11px;">('.$fechaDesde.' - '.$fechaHasta.')</span> <span style="font-size:12px;">$ '.number_format($tDocencia, 0, ',', '.').'</span>'; ?>
					</span>
			</div>
			<?php
				echo '<div>';
				...
				$filtroSQL = $filSQL;
				$bdHon=mysql_query("SELECT * FROM SolFactura $filtroSQL and pagoFactura <> 'on' Order By nSolicitud Desc");

				if($filtroSQL == ''){
					//$bdHon=mysql_query("SELECT * FROM SolFactura where pagoFactura <> 'on' Order By nSolicitud");
					$m = intval($MesNum[$MesFiltro]);
					$filtroSQL = "Where month(fechaSolicitud) <= $m and ";
					
					$filtroSQL = $filSQL;
					//$bdHon=mysql_query("SELECT * FROM SolFactura $filtroSQL and pagoFactura <> 'on' Order By fechaSolicitud Desc");
					$bdHon=mysql_query("SELECT * FROM SolFactura $filtroSQL and pagoFactura <> 'on' Order By nSolicitud Desc");
				}else{
					$m = intval($MesNum[$MesFiltro]);
					$filtroSQL = "Where month(fechaSolicitud) <= $m and ".$filtroSQL;
					//$bdHon=mysql_query("SELECT * FROM SolFactura $filtroSQL Order By nSolicitud" );

					$filtroSQL = $filSQL;
					//$bdHon=mysql_query("SELECT * FROM SolFactura $filtroSQL Order By fechaSolicitud Desc" );
					$bdHon=mysql_query("SELECT * FROM SolFactura $filtroSQL Order By nSolicitud Desc" );
				}
				if($dBuscado){
					$bdPer=mysql_query("SELECT * FROM Clientes Where Cliente Like '%".$dBuscado."%' or RutCli = '".$dBuscado."'");
					if ($rowP=mysql_fetch_array($bdPer)){
						$RutCli = $rowP[RutCli];
						$bdHon=mysql_query("SELECT * FROM SolFactura Where RutCli = '".$rowP[RutCli]."'");
						$dBuscado = '';
					}
				}
				if($nOrden != ''){
					$bdHon=mysql_query("SELECT * FROM SolFactura where nOrden Like '%".$nOrden."%'");
				}
				$filtroFacturas = "fechaSolicitud >= '".$fechaDesde."' and fechaSolicitud <= '".$fechaHasta."'";
				$bdHon=mysql_query("SELECT * FROM SolFactura Where $filtroFacturas");
				if ($row=mysql_fetch_array($bdHon)){
					do{
						//echo $row[RutCli].'<br>';
						$bdCF=mysql_query("SELECT * FROM Clientes Where cFree = 'on' and RutCli = '".$row['RutCli']."'");
						if($rowCF=mysql_fetch_array($bdCF)){
							...
							<table width="100%">
								...
								<tr>
									...
									<td>:
										<?php 
											$link=Conectarse();
											$bdDet=mysql_query("SELECT * FROM Clientes WHERE RutCli = '".$_GET['RutCli']."'");
											if($rowDet=mysql_fetch_array($bdDet)){
												$Cliente = $rowDet['Cliente'];
											}
											mysql_close($link);
								...
								<tr>
									...
									<td>:
										<?php 
											$link=Conectarse();
											$bdDet=mysql_query("SELECT * FROM SolFactura WHERE nSolicitud = '".$_GET['nSolicitud']."'");
											if($rowDet=mysql_fetch_array($bdDet)){
