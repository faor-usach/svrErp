...
<?php
	...
	if(isset($_GET['RAM']))  		{ $RAM  	 	= $_GET['RAM']; 		}
				...
				$tBruto = 0;
				$link=Conectarse();
				$bdHon=mysql_query("SELECT * FROM amInformes Where Estado = 'P' and CodInforme Like '%".$CodInforme."%' Order By CodInforme");
				if($row=mysql_fetch_array($bdHon)){
					do{
						...
						echo '		<td width="15%">';
										if($row['tpEnsayo']){
											$bdEns=mysql_query("SELECT * FROM amTpEnsayo Where tpEnsayo = '".$row['tpEnsayo']."'");
						...
						echo '		<td width="30%">';
										$cRes = 0;
										$iRes = 0;
										$bdUsr=mysql_query("SELECT * FROM Usuarios Where usr = '".$row['ingResponsable']."'");
										if($rowUsr=mysql_fetch_array($bdUsr)){
											$iRes = $rowUsr['titPie'].' '.$rowUsr['usuario'];
										}
										$bdUsr=mysql_query("SELECT * FROM Usuarios Where usr = '".$row['cooResponsable']."'");
										if($rowUsr=mysql_fetch_array($bdUsr)){
						...
						echo '		<td width="6%">';
										if($row['nMuestras']){
											$bdOT=mysql_query("SELECT * FROM OTAMs Where CodInforme = '".$row['CodInforme']."'");
