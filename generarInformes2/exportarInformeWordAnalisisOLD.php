...
if($tpEnsayo == '2'){
	if($version == 'Old'){
		$docx = new CreateDocxFromTemplate('plantillaINFORMESAnalisis.docx');
	}else{
		$docx = new CreateDocxFromTemplate('InformesAF.docx');
	}
}else{
	if($version == 'Old'){
		$docx = new CreateDocxFromTemplate('plantillaINFORMES.docx');
	}
	if($version == 'NewPortada'){
		$docx = new CreateDocxFromTemplate('InformesPortada.docx');
	}
	if($version == 'NewSimple'){
		$docx = new CreateDocxFromTemplate('InformesSimple.docx');
	}
}
$Rev = 0;
$options = array(
...
if($tpEnsayo == '4'){ $nomInforme = 'INFORME DE CARACTERIZACIÓN'; }
...
$txtDiv = '<div style="font-family:Arial; font-size:12px; text-align:Justify; margin:10px; margin-right:10px; padding:10px; text-indent: 10cm; line-height:180%;"';
...
/* HTML ERROR
$docx->replaceVariableByHTML('Metodologia', 'block',$txtDiv.$Metodologia.'</div>', array('parseDivsAsPs' => true));
$docx->replaceVariableByHTML('Objetivos', 	'block',$txtDiv.$Objetivos.'</div>', array('parseDivsAsPs' => true));
$docx->replaceVariableByHTML('Comentarios', 'block',$txtDiv.$Comentarios.'</div>', array('parseDivsAsPs' => true));
$docx->replaceVariableByHTML('Resumen', 	'block',$txtDiv.$Resumen.'</div>', array('parseDivsAsPs' => true));
...
if($rowNot=mysqli_fetch_array($bdNot)){
	...
	//$fe = explode('-', $rowNot['fechaRegistro']);
	$itemList[] = 'El ensayo de químico fue realizado el '.$fensayo; 
	$html .= '</ul>';
	$options = array(
						'font' 		=> 'Arial',
						'fontSize' 	=> 8,
