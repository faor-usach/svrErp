...
<?php
	...
	if(isset($_GET['Agno'])) 		{ $Agno 	 = $_GET['Agno']; 		}
				...
				//$bdHon=mysql_query("SELECT * FROM Cotizaciones Where (Estado = 'P' or Estado = 'T') and RAM > 0 Order By RAM");
				$bdHon=mysql_query("SELECT * FROM Cotizaciones Where Estado = 'P' and RAM > 0 Order By RAM");
				if($row=mysql_fetch_array($bdHon)){
					do{
						$tr = "bVerde";
						//$bdInf=mysql_query("SELECT * FROM Informes Where CodigoVerificacion != '' and CodInforme Like '%".$row[RAM]."%'");
						$bdInf=mysql_query("SELECT * FROM Informes Where informePDF != '' and CodInforme Like '%".$row[RAM]."%'");
						if($rowInf=mysql_fetch_array($bdInf)){
							...
							echo '		<td width="10%">';
											...
											$bdCot=mysql_query("SELECT * FROM amInformes Where CodInforme Like '%".$CodInforme."%' Order By CodInforme Desc");
